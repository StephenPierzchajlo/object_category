#--------------------------- Adaptive Bayesian Computation functions.
...
# Function for drawing samples from posterior distribution.
...
# Function for plotting posterior samples + credibility intervals.
abc_graph <- function(data, title) {
  ggplot(data = data[[1]], aes(x = posterior_unif)) +
    ...
    xlim(c(0, 1)) +
    geom_segment(aes(x = data[[2]][1], xend = data[[2]][2],
